...
export const ChallengeRule = (ctx, rule) => ({
    ctx,
    rule,
    execute: () => {
    ...
    postActionExecute: () => {
        const { nextGame, currentPlayer, arePromptActionsCompleted: isDone, allPromptActions } = ctx;
        ...
        if (isDone && allPromptActions.length === 1) {
            ...
            const losingPlayerId = candidatePlayerIds.find((id) => id !== winningPlayerId);
