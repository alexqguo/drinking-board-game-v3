...
const isDiceRollSuccessful = (cond, rolls) => {
    ...
    if (diceRolls && diceRolls.type === DiceRollType.allMatch) {
        return rolls.every((roll) => criteria.indexOf(roll) !== -1);
    }
    // Shouldn't happen, but let the player proceed if so
    return true;
};
export const ApplyMoveConditionRule = (ctx, rule) => ({
    ctx,
    rule,
    execute: () => {
        ...
        if (rule.condition?.immediate) {
            ...
            ctx.update_setPlayerActions(currentPlayer.id, actions);
        }
        if (!requiresActions) {
    ...
    postActionExecute: () => {
        const { arePromptActionsCompleted: isDone, allPromptActions, currentPlayer, nextGame, } = ctx;
        if (isDone) {
            if (rule.condition?.immediate) {
