...
const starterStrengths = Object.freeze({
  starter_totodile: 'starter_cyndaquil',
  starter_cyndaquil: 'starter_chikorita',
  starter_chikorita: 'starter_totodile',
});
    ...
    actions: {
      [ActionType.battleRoll]: () => ({
        execute: (ctx: any, args: Payloads[ActionType.battleRoll]) => {
          ctx.loggers.debug('In Gen 2 battle action execute!');
          const { actionId } = args;
    ...
    gameState: {
      [GameState.Battle]: (ctx: Context) => ({
        ...
        execute: () => {
          ...
          ctx.loggers.debug('In Gen 2 battle phase!');
